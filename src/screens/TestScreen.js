...
import PartyCard from "../components/TestPartyCard";
				...
				<FlatList
					...
					renderItem={ ({ item }) =>
						<DoubleTap onDoublePress={ () => this.enrollInParty(item.key) }
							...
							<View>
								<PartyCard title={ "test test test test test test test tests test test test test test test last" } date={ item.date } time={ item.time }
								           host={ item.host } imageURL={ item.imageURL } />
							</View>
						</DoubleTap> }
				/>
