...
const validate = values => {
    ...
    if (generalLocation !== undefined && generalLocation.length >= 50) {
        error.generalLocation = 'General Location must be less then 50 characters long';
    }
    if (date === undefined) {
        ...
        return (
            <View>
                ...
                <Item regular style={styles.formItem}>
                    <Button style={{backgroundColor: 'transparent'}} onPress={() => this.setState({showingDate: true})}>
                        <Text style={{
                            color: '#ee5253',
                            fontSize: 18,
                            fontWeight: '600',
    ...
    renderNameInput({input, label, type, meta: {touched, error, warning}}) {
        ...
        return (
            <Item regular style={{borderColor: '#000', marginLeft: 20}}>
                <Input placeholder="what's the plan?" style={styles.titleInput} selectionColor={'#ee5253'} {...input}
                       onChangeText={text => {
                           input.onChange(text);
    ...
    renderLocationInput({input, label, type, meta: {touched, error, warning}}) {
        ...
        return (
            <Item stackedLabel style={styles.formItem}>
                ...
                <Input placeholder="123 main street" style={styles.formInput} selectionColor={'#ee5253'} {...input}/>
            </Item>
        );
    }
